...
已实现 Agent 持久化任务启用覆盖框架：PiCli 候选任务在 Agent 加载阶段按 `config/agent_task_state.json` 标记启用状态，禁用候选仍保留在队列中；有效期结束或每日 05:00 后，在下一个 Agent 调度安全点重载并恢复执行，不用后台定时器中断业务任务。顶层任务及子节点均可覆盖；同入口的多个不同配置实例分别保留自己的 PiCli 基础 override，节点状态统一覆盖所有实例。已提供通用状态写入 action。
