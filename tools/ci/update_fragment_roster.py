#!/usr/bin/env python3
"""从 BWiki 刷新神秘商店勾玉碎片「待选项」（仅写 GUI checkbox cases）。

待选项唯一维护处
  assets/resource/tasks/日常任务.json
    → option「勾玉购买忍者碎片」.cases

Agent 不读这份名单：框架把用户勾选写进 attach，Agent 只买已选项。

用法
  # BWiki 作战忍者/作战角色已更新时（或 GitHub Actions 同名 workflow）
  python3 tools/ci/update_fragment_roster.py

  # BWiki 尚未收录新角色：直接改上面的 cases（追加一个 name + attach），发版即可。
"""
